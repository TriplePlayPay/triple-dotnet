...
{
    /// <summary>
    /// Standard client to make requests to Triple's API, using
    ...
    public class SystemNetHttpClient : IHttpClient
    {
        ...
        {
            // Enable support for TLS 1.2, as Triple's API requires it. This should only be
        ...
        {
            // We set the User-Agent and X-Triple-Client-User-Agent headers in each request
        ...
        public async Task<TripleResponse> MakeRequestAsync(
        ...
        public async Task<TripleStreamedResponse> MakeStreamingRequestAsync(
            TripleRequest request,
            CancellationToken cancellationToken = default)
        {
            var (response, retries) = await this.SendHttpRequest(request, cancellationToken).ConfigureAwait(false);

            return new TripleStreamedResponse(
                response.StatusCode,
                response.Headers,
                await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
            {
        ...
        {
            var userAgent = $"Triple/v1 .NetBindings";
            ...
            HttpHeaders headers)
        {
            ...
            if (headers != null && headers.Contains("Triple-Should-Retry"))
            {
                var value = headers.GetValues("Triple-Should-Retry").First();
            ...
            //
            // Note that we expect the Triple-Should-Retry header to be false
        ...
        private System.Net.Http.HttpRequestMessage BuildRequestMessage(TripleRequest request)
        {
